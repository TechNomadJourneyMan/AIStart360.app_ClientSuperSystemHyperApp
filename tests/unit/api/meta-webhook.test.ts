...
describe('/api/webhooks/meta', () => {
  beforeEach(() => {
    ...
    myhonorDelivery.apply.mockResolvedValue({
      matched: false,
      notificationId: null,
      state: null,
    })
  ...
  it('applies a MyHonor transactional delivery status without creating a generic placeholder', async () => {
    process.env.META_APP_SECRET = 'app-secret'
    myhonorDelivery.apply.mockResolvedValue({
      matched: true,
    ...
    expect(response.status).toBe(200)
    expect(myhonorDelivery.apply).toHaveBeenCalledWith(expect.objectContaining({
