...
- [x] **Persist the workspace compatibly.** Added `supabase/migrations/074_ai_first_workspace.sql` for journey workspaces, messages and file metadata, followed by `075_ai_journey_device_sync.sql` for hash-only device credentials, one-time connect codes and server-owned CAS revisions. Browser-local persistence remains the explicit local fallback when the migration/service role is unavailable.
