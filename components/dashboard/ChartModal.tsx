...
export function ChartModal({ metric, onClose }: ChartModalProps) {
  ...
  const METRIC_LABELS: Record<string, string> = {
    revenue:   t('dashboard.kpiMetrics.revenue'),
    margin:    t('dashboard.kpiMetrics.margin'),
    clients:   t('dashboard.kpiMetrics.clients'),
    avg_check: t('dashboard.kpiMetrics.avgCheck'),
  }
  useEffect(() => {
    if (!metric) return
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div>
                <p className="text-[10px] font-mono text-on-surface-variant uppercase tracking-widest mb-0.5">{t('dashboard.chart.dynamics')}</p>
