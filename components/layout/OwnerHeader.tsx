...
import { OWNER_PRIMARY_NAV, OWNER_SECONDARY_NAV, isOwnerNavActive } from './OwnerSidebar'
...
export function OwnerHeader() {
  ...
  const allNav = [...OWNER_PRIMARY_NAV, ...OWNER_SECONDARY_NAV]
  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-[220px] z-40 h-16 bg-[#0e0f14]/80 backdrop-blur-xl border-b border-white/[0.04] flex items-center justify-between px-4 md:px-6">
      ...
      {/* Right */}
      <div className="flex items-center gap-3">
        <Link href="/owner/notifications" aria-label="Уведомления"
        ...
        <div className="lg:hidden fixed inset-0 z-50" role="dialog" aria-modal="true">
          ...
          <div className="absolute left-0 top-0 h-full w-[260px] flex flex-col bg-[#0e0f14] border-r border-white/[0.04]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.04]">
              <span className="text-xs font-mono text-secondary/60 uppercase tracking-[0.2em]">Owner Portal</span>
                ...
                return (
                  <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)}
                    className={`flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all ${
                      active ? 'bg-secondary/10 text-secondary' : 'text-[#8a93a0] hover:text-[#e6f1ea] hover:bg-white/[0.04]'
                    }`}>
