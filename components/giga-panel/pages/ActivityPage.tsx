...
export function ActivityPage() {
  ...
  const columns: Column<EventRow>[] = [
    ...
    { key: 'user', header: 'Пользователь', render: (e) => e.user_id ? <Link href={`${base}/users/${e.user_id}?tab=activity`} className="text-slate-200 hover:text-blue-300" onClick={(ev) => ev.stopPropagation()}>{e.user?.full_name || e.user?.email || e.user_id.slice(0, 8)}</Link> : '—' },
  ...
  return (
    <RequirePermission permission="activity.view">
      <PageHeader
        crumbs={[{ label, href: base }, { label: 'Активность' }]}
