...
export function InvitesPage() {
  ...
  // Роль в приглашении = выдача прав, поэтому предлагаем её только тем, кто
  // и так управляет ролями. Сервер проверяет то же самое ещё раз.
    ...
    try {
      const res = await gigaFetch<{ sent: number; failed: number; results: InviteResult[] }>('/api/giga-admin/invites', {
        ...
        json: {
          ...
          staffRole: canAssignRole && staffRole ? staffRole : undefined,
        },
      })
  ...
  const columns: Column<InviteRow>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex flex-wrap items-center gap-1">
          ...
          {r.staffRoleLabel && <Badge tone="violet">{r.staffRoleLabel}</Badge>}
        </div>
      ),
    },
    ...
    { key: 'by', header: 'Кто пригласил', render: (r) => <span className="text-slate-500">{r.sentBy === 'giga:super_admin' ? 'общий пароль' : r.sentBy}</span> },
    {
      key: 'user', header: '',
      render: (r) => (r.userId
        ? <Link href={`${base}/users/${r.userId}`} className="text-[11px] text-blue-300 hover:underline">Карточка</Link>
  ...
  return (
    <RequirePermission permission="users.view">
      <PageHeader
        crumbs={[{ label, href: base }, { label: 'Приглашения' }]}
      ...
      {canInvite && (
        <Panel className="mb-4" title={<span className="flex items-center gap-2"><Mail size={14} className="text-blue-300" /> Новое приглашение</span>}>
          ...
          <div className="mt-3 grid gap-3 lg:grid-cols-2">
            <Field label="Компания (необязательно)" hint="Попадёт в письмо и в запись приглашения.">
              <input
                value={company}
            ...
            {canAssignRole && (
              <Field label="Роль на платформе (необязательно)" hint="Роль выдаётся автоматически, когда человек примет приглашение.">
                <select
                  value={staffRole}
                ...
                >
                  ...
                  {(Object.keys(STAFF_ROLE_LABELS) as StaffRole[])
                    ...
                    .map((r) => <option key={r} value={r}>{STAFF_ROLE_LABELS[r]}</option>)}
                </select>
              </Field>
            )}
          </div>
