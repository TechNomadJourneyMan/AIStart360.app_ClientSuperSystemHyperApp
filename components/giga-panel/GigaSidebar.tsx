...
export function GigaSidebar({ isOpen = false, onClose }: GigaSidebarProps) {
  ...
  const logout = async () => {
    try {
      ...
      window.location.href = me?.kind === 'break_glass' ? '/giga-login' : '/login'
    }
  }
  ...
  const content = (
    <>
      <div className="flex items-center justify-between px-5 pb-4 pt-6">
        <Link href="/admin-giga-panel" className="flex items-center gap-3" onClick={onClose}>
          ...
          <div>
            ...
            <p className="mt-0.5 text-[10px] text-slate-500">Platform Control Center</p>
          </div>
        </Link>
