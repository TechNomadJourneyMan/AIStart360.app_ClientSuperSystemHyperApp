...
import { GigaSidebar } from './GigaSidebar'
import { StaffProvider, useStaff } from './StaffContext'
import { WorkspaceProvider, type Workspace } from './WorkspaceContext'

/**
 * Общая оболочка рабочих кабинетов персонала (GIGA-CRM и SuperExpert).
 *
 * Одна вёрстка, одна боковая панель, один загрузчик прав — различия
 * (базовый путь, меню, подпись, куда вести при отказе) приходят в `workspace`.
 */
function ShellBody({ workspace, children }: { workspace: Workspace; children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <div className="fixed left-0 right-0 top-0 z-40 flex items-center gap-3 border-b border-white/[0.07] bg-slate-950/90 px-4 py-3 backdrop-blur-xl md:hidden">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.15em] text-blue-400">{workspace.label}</p>
      </div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-10 pt-20 md:px-8 md:pt-8">
          {!loading && !me && error ? (
            <div className="rounded-2xl border border-red-500/25 bg-red-500/10 p-6 text-center">
              <p className="text-sm font-semibold text-red-200">Нет доступа к кабинету</p>
              <p className="mt-1 text-xs text-slate-400">{error}</p>
              <a href={workspace.loginPath} className="mt-4 inline-block rounded-xl bg-blue-500 px-4 py-2 text-xs font-semibold text-white">Войти</a>
            </div>
          ) : children}
        </div>
      </main>
...
export function WorkspaceShell({ workspace, children }: { workspace: Workspace; children: React.ReactNode }) {
  return (
    <WorkspaceProvider workspace={workspace}>
      <StaffProvider>
        <ShellBody workspace={workspace}>{children}</ShellBody>
      </StaffProvider>
    </WorkspaceProvider>
  )
}
