...
export async function POST(request: Request) {
  // Throttle: this endpoint creates real (approved) Supabase accounts with the
  // service-role key. Without a cap it can be abused to mass-provision accounts.
  if (await isRateLimited(request, 'demo-access', { max: 5 })) {
    return NextResponse.json(
      { ok: false, error: 'rate_limited', detail: 'Слишком много запросов. Попробуйте позже.' },
      { status: 429 },
    )
  }
  ...
  return NextResponse.json(
    {
      ...
      password,
      userId,
      // Demo users land on the welcome screen: pick vertical (тип бизнеса),
      // then pick the start path (анкета / загрузка файлов / просто посмотреть).
      // Full new-account modelling — same flow as a fresh registration.
