...
import { reverseExpense } from '@/lib/sales-monitoring/expense-service'
import { idempotentResponse, parseJson } from '@/lib/sales-monitoring/http'

const schema = z.object({
  organizationId: z.string().min(1),
  reason: z.string().min(3).max(1000),
  reversedAt: z.string().date().optional(),
})
...
export async function POST(
  ...
  try {
    ...
    const access = await requireSalesPermission(input.organizationId, 'expenses:reverse')
    const result = await reverseExpense(
      { ...input, expenseId: params.id },
      access,
