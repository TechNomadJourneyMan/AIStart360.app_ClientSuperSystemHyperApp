        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="w-48 h-48 mb-8 relative">
            <svg viewBox="0 0 200 200" className="w-full h-full">
              ...
              <text x="100" y="115" textAnchor="middle" fill="#34d399" fontSize="9" letterSpacing="3" opacity="0.7">{t('auth.platform')}</text>
            </svg>
          </div>

          <h2 className="text-lg font-mono text-on-surface-variant/70 tracking-widest uppercase">{t('auth.aiExperts')}</h2>
          <h1 className="text-2xl font-headline font-extrabold text-primary mt-1">{t('auth.growthPlatform')}</h1>
          ...
          <div className="flex gap-3 mt-6">
            {[['500+', t('auth.diagnosticsCount')], ['27', t('auth.industriesCount')], ['СНГ + MENA', '']].map(([val, label], i) => (
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-[10px] font-mono text-primary/80 uppercase tracking-[0.2em]">{t('auth.institutionalAccess')}</span>
          </div>

          <h1 className="text-2xl font-headline font-extrabold text-on-surface mb-1">{t('auth.signIn')}</h1>
          <p className="text-sm text-on-surface-variant/60 mb-8">{t('auth.enterCredentials')}</p>
          ...
          >
            ...
            <span className="text-sm font-medium">{t('auth.demoAccess')}</span>
          </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center ml-1">
                <label className="block text-[10px] font-mono text-on-surface-variant/60 uppercase tracking-widest">{t('auth.password')}</label>
                <Link href="/forgot-password" className="text-[11px] text-primary/70 hover:text-primary transition-colors">{t('auth.forgotPassword')}</Link>
              </div>
              <div className="relative group">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center">
              <span className="px-4 text-[10px] font-mono text-on-surface-variant/40 bg-[#0a0e17] uppercase tracking-widest">{t('common.or')}</span>
            </div>
          </div>
