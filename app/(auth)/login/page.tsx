...
function LoginContent() {
  ...
  const onSubmit = async (data: Form) => {
    ...
    if (role === 'client') {
      ...
      router.replace(
        from.startsWith('/expert') || from.startsWith('/owner') ? '/dashboard' : from
      )
    } else if (role === 'owner') {
      ...
      router.replace('/admin-giga-panel')
    } else {
      // Unknown role — fallback to dashboard
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-8 lg:p-12 overflow-x-hidden">
        <div className="w-full max-w-[400px]">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button
              type="button"
              className="flex items-center justify-center gap-2 py-3 rounded-xl border border-white/[0.08] bg-surface-container hover:bg-surface-container-high text-on-surface text-sm transition-colors opacity-50 cursor-not-allowed"
              disabled
              title="Скоро"
            >
