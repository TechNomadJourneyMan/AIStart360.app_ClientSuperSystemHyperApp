...
export default function GriAdminPage() {
  ...
  return (
    <RequirePermission permission="gri.view">
      <PageHeader
        crumbs={[{ label: 'GIGA-CRM', href: '/admin-giga-panel' }, { label: 'GRI' }]}
      ...
      {!d ? <Skeleton className="h-80" /> : (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <StatTile label="Клиентов с результатом" value={d.stats.users} href="/admin-giga-panel/users?segment=gri_completed" />
            <StatTile label="Средний индекс" value={d.stats.avgIndex ?? '—'} hint="по текущим результатам" tone="violet" />
            <StatTile label="Проходят сейчас" value={d.stats.drafts} tone="amber" href="/admin-giga-panel/users?segment=gri_in_progress" />
          ...
          >
            <DataTable columns={columns} rows={d.list} rowKey={(r) => r.id} loading={loading} onRowClick={(r) => router.push(`/admin-giga-panel/users/${r.user_id}?tab=gri`)} />
              ...
              <ul className="divide-y divide-white/[0.05]">
                {d.drafts.map((x) => (
                  <li key={x.user_id} className="flex items-center justify-between py-2 text-xs">
                    <Link href={`/admin-giga-panel/users/${x.user_id}?tab=gri`} className="text-slate-200 hover:text-blue-300">{x.user?.organization || x.user?.full_name || x.user?.email || x.user_id}</Link>
