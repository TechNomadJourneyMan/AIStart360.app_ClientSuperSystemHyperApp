...
|---|---|---|---|---|---|
| D1 | **Critical** | `lib/get-dashboard-data.ts:12-48` | Returns hardcoded **Choco** KPI/alerts/signals/competitors for **every** user (the `isChoco` branch and the fallthrough are identical). Renders fabricated companies (Vortex Labs, Calyx Digital, Kaspi $5.6B) on `/insights`, `/intelligence`, `/competitors`. | Replace with real org‑scoped data or empty state; gate Choco behind explicit demo flag. | ☑ |
