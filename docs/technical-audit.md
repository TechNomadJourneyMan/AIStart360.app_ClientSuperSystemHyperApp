...
| A2 | **Critical** | `app/api/giga-admin/auth/route.ts` | Super‑admin via plaintext env password, no rate‑limit, cookie `httpOnly:false`. | Added rate‑limit (Upstash + in‑memory fallback, 429), `httpOnly:true` + `secure` in prod. The "must be JS‑readable" comment was false — nothing reads it client‑side. | ☑ |
| A2b | **Critical** | giga‑admin guards (`req.cookies.get('aistart360_role')==='super_admin'`) | The role cookie is an **unsigned static string**; a user can craft a request with `aistart360_role=super_admin` and gain full giga‑admin (httpOnly doesn't stop request‑crafting). | **Sign the cookie** (HMAC over server secret) + verify in all readers (middleware + giga‑admin routes + me). Touches ~8 sites incl. the shared role cookie → needs its own focused change. | ☐ |
| A3 | High | `app/api/auth/register/route.ts` | `role` accepted from client incl. `admin`; **non‑client roles auto‑approved** (`status = role==='client' ? pending : approved`) → self‑register as approved owner/admin. | Restricted schema to `client`/`owner`; **always `pending_approval`** (no auto‑approve); admin/expert can't self‑register. `email_confirm` left (approval gate covers it). | ☑ |
| A4 | High | `app/api/dev/register`; `app/api/dev/confirm-email` | Dev backdoors (mint any role incl. super_admin) gated only by `NODE_ENV!=='production'`. | Now require `NODE_ENV!=='production'` **AND** `ENABLE_DEV_AUTH_ROUTES==='1'`; off by default everywhere. | ☑ |
...
| A6 | High | `app/giga-login/page.tsx` | Super‑admin password stored in `localStorage`. | Removed all localStorage persistence + the "remember password" checkbox; rely on the browser password manager. | ☑ |
